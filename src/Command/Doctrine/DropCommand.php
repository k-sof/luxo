...
class DropCommand extends Command
{
    ...
    public function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$input->getOption('force')) {
            ...
            $output->writeln('');
            $output->writeln(sprintf('<info>Would drop the database <comment>%s</comment></info>', $name));
        ...
        try {
            if ($shouldDropDatabase) {
                ...
                $output->writeln(sprintf('<info>Database <comment>%s</comment> doesn\'t exist. Skipped.</info>', $name));
            }
    ...
    protected function configure()
    {
        $this
            ...
            ->setHelp(<<<'EOT'
The <info>%command.name%</info> command drop database:
